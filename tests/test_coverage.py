...
def test_make_meta():
    ...
    redpy.make_meta(path='tests/out', topath='.', verbose=True)
    assert os.path.getsize('tests/out/meta.html')


def test_initialize():
    """Confirm redpy-initialize makes empty .h5 file."""
    print_section_header('redpy-initialize')
    redpy.initialize(configfile='tests/test0.cfg', verbose=True)
    assert check_table_lengths(configfile='tests/test0.cfg')
...
def test_plot_junk():
    ...
    redpy.plot_junk(configfile='tests/test0.cfg', verbose=True)
    assert os.path.exists('tests/out/test/junk/20040923232035-kurt.png')
    assert os.path.exists('tests/out/test/junk/20040923232730-both.png')
    assert os.path.exists('tests/out/test/junk/20040923233515-freq.png')
    assert os.path.getsize('tests/out/test/junk.txt')


def test_clear_junk():
    """Confirm redpy-clear-junk clears the junk table."""
    print_section_header('redpy-clear-junk')
    redpy.clear_junk(configfile='tests/test0.cfg', verbose=True)
    assert check_table_lengths(
        configfile='tests/test0.cfg', lengths=[2, 2, 0, 0, 0, 0, 0])
...
def test_catfill_force():
    ...
    assert check_table_lengths(
        configfile='tests/test0.cfg', lengths=[6, 3, 3, 1, 0, 0, 2])
...
def test_compare_catalog():
    ...
    redpy.compare_catalog(
        './tests/out/testcat.csv', arrival=False, configfile='tests/test0.cfg',
        delimiter=',', include_missing=True, junk=True, maxdtoffset=0.1,
        name='Arrival', outfile='./tests/out/matches.csv', verbose=True)
    df = pd.read_csv('./tests/out/matches.csv')
...
def test_remove_family():
    ...
    redpy.remove_family(0, configfile='tests/test0.cfg', verbose=True)
    assert check_table_lengths(
        configfile='tests/test0.cfg', lengths=[6, 3, 0, 0, 0, 1, 0])


def test_backfill_deleted():
    """Confirm redpy-backfill checks against deleted family."""
    print_section_header('redpy-backfill deleted')
    redpy.backfill(configfile='tests/test0.cfg', verbose=True,
                   starttime='2004-09-23T23:00',
                   endtime='2004-09-24T00:00', nsec=1800)
    assert check_table_lengths(
        configfile='tests/test0.cfg', lengths=[11, 8, 0, 0, 16, 1, 0])
...
def test_backfill_from_file():
    ...
    redpy.backfill(configfile='tests/test1.cfg', verbose=True,
                   starttime='2004-10-01T00:00',
                   endtime='2004-10-01T01:00', nsec=3600)
    assert check_table_lengths(
        configfile='tests/test1.cfg', lengths=[21, 10, 0, 0, 16, 1, 0])
...
def test_distant_families(capsys):
    ...
    redpy.distant_families(
        configfile='tests/test1.cfg', verbose=True, findphrase='Amboy',
        percent=90)
...
def test_write_family_locations():
    ...
    df = pd.read_csv('tests/out/test/famlocs.csv')
    assert len(df) == 23
...
def test_create_report():
    ...
    assert os.path.getsize('tests/out/test/reports/0-report.html')
    ...
    assert os.path.getsize('tests/out/test/reports/7-ccc_full.npy')
    assert os.path.getsize('tests/out/test/reports/7-evTimes.npy')
    assert os.path.getsize('tests/out/test/reports/7-report.html')
...
def test_create_pdf_family():
    ...
    assert os.path.getsize('tests/out/test/families/fam7.pdf')
...
def test_create_pdf_overview():
    ...
    redpy.create_pdf_overview(
        ...
        starttime='2004-10-01T06:00', endtime='2004-10-01T20:00',
        minmembers=0, occurheight=1,
        plotformat='eqrate,fi,occurrence,occurrencefi,longevity')
    redpy.create_pdf_overview(
        configfile='tests/test1.cfg', verbose=True, usehrs=False,
        minmembers=5, occurheight=1)
    assert os.path.getsize('tests/out/test/overview.pdf')
...
def test_extend_table():
    ...
    detector.close()
    redpy.extend_table(
        'tests/test1.cfg', 'tests/test2.cfg', verbose=True, noplot=False)
    detector = redpy.Detector('tests/test2.cfg', opened=True)
...
def test_force_plot():
    ...
    redpy.force_plot(
        configfile='tests/test2.cfg', famplot=True, startfam=-2, endfam=-1)
    assert not os.path.exists('tests/out/test/families/6.png')
    assert not os.path.exists('tests/out/test/families/0.png')
    assert not os.path.exists('tests/out/test/families/5.html')
    assert os.path.getsize('tests/out/test/families/5.png')
    redpy.force_plot(
        configfile='tests/test2.cfg', famplot=True, startfam=6)
    assert not os.path.exists('tests/out/test/families/0.png')
    ...
    assert os.path.getsize('tests/out/test/families/0.png')
