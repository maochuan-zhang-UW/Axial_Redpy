...
top20 = sorted(fam_sizes, key=lambda x: -x[1])[:20]
...
for rank, (fam_idx, size) in enumerate(top20):
...
ax.set_title('Top 20 Repeating Earthquake Families — Axial Seamount Mar 2015', fontsize=13)
ax.xaxis.set_major_formatter(mdates.DateFormatter('%b %d'))
ax.xaxis.set_major_locator(mdates.DayLocator(interval=3))
plt.xticks(rotation=45)
ax.set_yticks(range(1, 21))
ax.set_yticklabels([f'#{r} (n={s})' for r, (i, s) in enumerate(top20, 1)])
ax.grid(True, alpha=0.3)
ax.legend(loc='upper left', fontsize=8, ncol=2)
plt.tight_layout()

outfile = './runs/axial/top20_families.png'
